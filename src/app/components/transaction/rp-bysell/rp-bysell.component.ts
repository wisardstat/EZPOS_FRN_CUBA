...
@Component({
  selector: 'app-rp-bysell',
  templateUrl: './rp-bysell.component.html',
  styleUrls: ['./rp-bysell.component.scss']
})
export class RpBysellComponent {
  ...
  ngOnInit() { 
    ...
    this.type_doc_sv.getList().subscribe(response =>
      {  
        ...
        this._typedoc_list = this._typedoc_list.filter(item => item.type_doc_id === "IV");
        console.log(this._typedoc_list)
  ...
  public resetFind()
  {
    // this._typedoc_list.filter(
    //   type_doc => type_doc.type_doc_id === "IV");
    // Set default control search
    this.selected_wh = 'W001'
    this.selected_typeDoc = "IV"
