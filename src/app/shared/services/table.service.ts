...
import { StockcardDB } from '../data/tables/stockcard';
...
function sort(tableItem: StockcardDB[], column: SortColumn, direction: string): StockcardDB[] {
    if (direction === '' || column === '') {
        return tableItem;
    } else {
...
function matches(table: StockcardDB, term: string, pipe: PipeTransform) {
    return table.cust_fname.toLowerCase().includes(term.toLowerCase())
    || pipe.transform(table.doc_id).includes(term)
